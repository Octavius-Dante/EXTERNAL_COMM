...
@AbapCatalog.sqlViewName: 'ZP2P_VN_ANA'
...
define  view  ZMM_CDS_VENDOR_ANA
...
left outer join  ekpo on ekpo.ebeln = bseg.ebeln and ekpo.ebelp = bseg.ebelp and  bseg.ebeln <> ''
left outer join  ekko on ekko.ebeln = bseg.ebeln  and bseg.ebeln <> ''
left outer join mara on bseg.matnr = mara.matnr
//
left outer join t001 on t001.bukrs = bseg.bukrs
left outer join /opt/vim_1head as vn on vn.bukrs =  bseg.bukrs and vn.belnr = bseg.belnr and vn.gjahr = bseg.gjahr and vn.belnr <> ''
...
//left outer join  bseg as vn_bg2 on bseg.bukrs = vn_bg2.bukrs and bseg.belnr  = vn_bg2.belnr and bseg.gjahr  = vn_bg2.gjahr and ( vn_bg2.projk is not initial or vn_bg2.kostl <> '' or vn_bg2.aufnr <> '' )

left outer join aufk on bseg.aufnr= aufk.aufnr 
left outer join prps on  prps.pspnr  = aufk.objnr

left outer join skat on skat.ktopl = t001.ktopl and bseg.hkont = skat.saknr and skat.spras = $session.system_language
...
currency_conversion( amount => bseg.skfbt , source_currency => pswsl, target_currency => cast('USD' as abap.cuky(5)), exchange_rate_date =>bkpf.budat  , error_handling => 'SET_TO_NULL' ) as SKFBT_CAD,
...
case when ekpo.afnam <> '' then ekpo.afnam else vn.requisitioner   end as requisitioner,
 //VENDOR
 bseg.lifnr  as Vendor, //VENDOR

skat.txt50 as GL_TEXT,
//mbew.salk3 ,//inventory value
case when
   bseg.hkont =  '0000200003'
or  bseg.hkont = '0000200001'
or  bseg.hkont = '0000200025'
or  bseg.hkont = '0000200026'
or  bseg.hkont = '0000200031'
or  bseg.hkont = '0000200030'

 then 1 else 0 end as CNT_INV,
 t001.butxt,
 //WBS element
 prps.poski ,
...
////Migrated to latest version ,keeping the code same in both the systems
//where bkpf.budat  >=  DATS_ADD_MONTHS(   TSTMP_TO_DATS(
